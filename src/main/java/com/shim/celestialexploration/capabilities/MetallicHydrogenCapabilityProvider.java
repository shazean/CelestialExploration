...
public class MetallicHydrogenCapabilityProvider implements ICapabilityProvider, ICapabilitySerializable<CompoundTag> {

    public static Capability<FuelTankHandler> METALLIC_HYDROGEN_TANK = CapabilityManager.get(new CapabilityToken<>() {});
    ...
    private FuelTankHandler createFuelHandler() {
        if (fuelTankHandler == null) {
            fuelTankHandler = new FuelTankHandler(FuelUtil.HYDRO_FUEL_SPEED, FuelUtil.HYDRO_CAPACITY_MODIFIER);
        }
