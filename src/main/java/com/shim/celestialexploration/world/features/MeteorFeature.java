...
public class MeteorFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public MeteorFeature(Codec<NoneFeatureConfiguration> p_65865_) {
        super(p_65865_);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159861_) {
        ...
        if (!worldgenlevel.isEmptyBlock(blockpos)) {
            ...
            if (!blockstate.is(BlockRegistry.MOON_SAND.get())) {
                ...
                worldgenlevel.setBlock(blockpos, BlockRegistry.GLOWING_MOON_SAND.get().defaultBlockState(), 2);

                for(int i = 0; i < 1500; ++i) {
                    BlockPos blockpos1 = blockpos.offset(random.nextInt(3) - random.nextInt(3), -random.nextInt(5), random.nextInt(3) - random.nextInt(3));
