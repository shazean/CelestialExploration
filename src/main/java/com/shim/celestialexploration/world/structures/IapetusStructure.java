...
public class IapetusStructure extends StructureFeature<JigsawConfiguration> {
    ...
    public IapetusStructure() {
        super(CODEC, IapetusStructure::createPiecesGenerator, PostPlacementProcessor.NONE);
    }
    ...
    private static boolean isFeatureChunk(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        ...
        Vec3 coordinates = CelestialUtil.getPlanetaryChunkCoordinates(6);

        return chunkpos.x == coordinates.x - 5 && chunkpos.z == coordinates.z - 5;
    }
    ...
    public static @NotNull Optional<PieceGenerator<JigsawConfiguration>> createPiecesGenerator(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        ...
        if (!IapetusStructure.isFeatureChunk(context)) {
            return Optional.empty();
        }

        BlockPos blockpos = context.chunkPos().getMiddleBlockPosition(0);
        blockpos = new BlockPos(blockpos.getX(), 64 - 16, blockpos.getZ());
        ...
        if(structurePiecesGenerator.isPresent()) {
            CelestialExploration.LOGGER.log(Level.DEBUG, "Iapetus at {}", blockpos);
        }
