...
public class OceanJigsawPlacement extends JigsawPlacement {
static final Logger LOGGER = LogUtils.getLogger();

public static Optional<PieceGenerator<JigsawConfiguration>> addPieces(PieceGeneratorSupplier.Context<JigsawConfiguration> configSupplier, OceanJigsawPlacement.PieceFactory pieceFactory, BlockPos pos, boolean isVillage, boolean shouldApplyYToWorldHeight) {
    WorldgenRandom worldgenrandom = new WorldgenRandom(new LegacyRandomSource(0L));
    worldgenrandom.setLargeFeatureSeed(configSupplier.seed(), configSupplier.chunkPos().x, configSupplier.chunkPos().z);
    RegistryAccess registryaccess = configSupplier.registryAccess();
    JigsawConfiguration jigsawconfiguration = configSupplier.config();
    ChunkGenerator chunkgenerator = configSupplier.chunkGenerator();
    StructureManager structuremanager = configSupplier.structureManager();
    LevelHeightAccessor levelheightaccessor = configSupplier.heightAccessor();
    Predicate<Holder<Biome>> predicate = configSupplier.validBiome();
    ...
    if (structurepoolelement == EmptyPoolElement.INSTANCE) {
        ...
        int y;
        if (shouldApplyYToWorldHeight) {
            y = pos.getY() + chunkgenerator.getFirstFreeHeight(x, z, Heightmap.Types.OCEAN_FLOOR_WG, levelheightaccessor);
        } else {
            y = pos.getY();
        }

        if (!predicate.test(chunkgenerator.getNoiseBiome(QuartPos.fromBlock(x), QuartPos.fromBlock(y), QuartPos.fromBlock(z)))) {
            return Optional.empty();
        } else {
            ...
            return Optional.of((builder, context) -> {
                ...
                if (jigsawconfiguration.maxDepth() > 0) {
                    int i1 = 80;
                    AABB aabb = new AABB((double)(x - 80), (double)(y - 80), (double)(z - 80), (double)(x + 80 + 1), (double)(y + 80 + 1), (double)(z + 80 + 1));
                    OceanJigsawPlacement.Placer placer = new OceanJigsawPlacement.Placer(registry, jigsawconfiguration.maxDepth(), pieceFactory, chunkgenerator, structuremanager, list, worldgenrandom);
                    placer.placing.addLast(new OceanJigsawPlacement.PieceState(poolelementstructurepiece, new MutableObject<>(Shapes.join(Shapes.create(aabb), Shapes.create(AABB.of(boundingbox)), BooleanOp.ONLY_FIRST)), 0));

                    while(!placer.placing.isEmpty()) {
                        OceanJigsawPlacement.PieceState jigsawplacement$piecestate = placer.placing.removeFirst();
                        placer.tryPlacingChildren(jigsawplacement$piecestate.piece, jigsawplacement$piecestate.free, jigsawplacement$piecestate.depth, isVillage, levelheightaccessor);
                    }
...
public static void addPieces(RegistryAccess access, PoolElementStructurePiece p_210292_, int p_210293_, OceanJigsawPlacement.PieceFactory factory, ChunkGenerator chunkGenerator, StructureManager structureManager, List<? super PoolElementStructurePiece> pieces, Random rand, LevelHeightAccessor accessor) {
    Registry<StructureTemplatePool> registry = access.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY);
    OceanJigsawPlacement.Placer jigsawplacement$placer = new OceanJigsawPlacement.Placer(registry, p_210293_, factory, chunkGenerator, structureManager, pieces, rand);
    jigsawplacement$placer.placing.addLast(new OceanJigsawPlacement.PieceState(p_210292_, new MutableObject<>(Shapes.INFINITY), 0));

    while(!jigsawplacement$placer.placing.isEmpty()) {
        OceanJigsawPlacement.PieceState jigsawplacement$piecestate = jigsawplacement$placer.placing.removeFirst();
        jigsawplacement$placer.tryPlacingChildren(jigsawplacement$piecestate.piece, jigsawplacement$piecestate.free, jigsawplacement$piecestate.depth, false, accessor);
    }

}

public interface PieceFactory {
    PoolElementStructurePiece create(StructureManager structureManager, StructurePoolElement poolElement, BlockPos pos, int p_210304_, Rotation rotation, BoundingBox boundingBox);
}
...
static final class Placer {
    ...
    private final OceanJigsawPlacement.PieceFactory factory;
    ...
    Placer(Registry<StructureTemplatePool> pools, int maxDepth, OceanJigsawPlacement.PieceFactory factory, ChunkGenerator chunkGen, StructureManager structureManager, List<? super PoolElementStructurePiece> pieces, Random random) {
        this.pools = pools;
    ...
    void tryPlacingChildren(PoolElementStructurePiece piece, MutableObject<VoxelShape> p_210335_, int p_210336_, boolean p_210337_, LevelHeightAccessor accessor) {
        ...
        for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo : structurepoolelement.getShuffledJigsawBlocks(this.structureManager, blockpos, rotation, this.random)) {
            ...
            if (optional.isPresent() && (optional.get().size() != 0 || Objects.equals(resourcelocation, Pools.EMPTY.location()))) {
                ...
                if (optional1.isPresent() && (optional1.get().size() != 0 || Objects.equals(resourcelocation1, Pools.EMPTY.location()))) {
                            ...
                            for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 : list1) {
                                if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                    ...
                                    if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                        ...
                                        piece.addJunction(new JigsawJunction(blockpos2.getX(), l2 - j + i3, blockpos2.getZ(), k1, structuretemplatepool$projection1));
                                        ...
                                        if (p_210336_ + 1 <= this.maxDepth) {
                                            this.placing.addLast(new OceanJigsawPlacement.PieceState(poolelementstructurepiece, mutableobject1, p_210336_ + 1));
                                        }
                                        continue label139;
                                    }
                                }
                    ...
                    OceanJigsawPlacement.LOGGER.warn("Empty or non-existent fallback pool: {}", (Object)resourcelocation1);
                }
            } else {
                OceanJigsawPlacement.LOGGER.warn("Empty or non-existent pool: {}", (Object)resourcelocation);
            }
        }
