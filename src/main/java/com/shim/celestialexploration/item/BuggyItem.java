...
public class BuggyItem extends Item {
    ...
    public BuggyItem(Item.Properties p_40620_) {
        super(p_40620_);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (hitresult.getType() == HitResult.Type.MISS) {
            ...
            if (hitresult.getType() == HitResult.Type.BLOCK) {
                Buggy buggy = new Buggy(level, hitresult.getLocation().x, hitresult.getLocation().y, hitresult.getLocation().z);
                buggy.setYRot(player.getYRot());
                if (!level.noCollision(buggy, buggy.getBoundingBox())) {
