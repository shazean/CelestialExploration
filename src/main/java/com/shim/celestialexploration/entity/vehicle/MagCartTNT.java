...
public class MagCartTNT extends AbstractMagCart {
    private static final byte EVENT_PRIME = 10;
    private int fuse = -1;

    public MagCartTNT(EntityType<? extends MagCartTNT> p_38649_, Level p_38650_) {
        super(p_38649_, p_38650_);
    }

    public MagCartTNT(Level p_38652_, double p_38653_, double p_38654_, double p_38655_) {
        super(CelestialEntities.TNT_MAGCART.get(), p_38652_, p_38653_, p_38654_, p_38655_);
    }

    public AbstractMagCart.Type getMagCartType() {
        return AbstractMagCart.Type.TNT;
    }
    ...
    public void activateMagCart(int p_38659_, int p_38660_, int p_38661_, boolean p_38662_) {
    ...
    public float getBlockExplosionResistance(Explosion p_38675_, BlockGetter p_38676_, BlockPos p_38677_, BlockState p_38678_, FluidState p_38679_, float p_38680_) {
        return !this.isPrimed() || !p_38678_.is(CelestialTags.Blocks.MAG_RAIL) && !p_38676_.getBlockState(p_38677_.above()).is(CelestialTags.Blocks.MAG_RAIL) ? super.getBlockExplosionResistance(p_38675_, p_38676_, p_38677_, p_38678_, p_38679_, p_38680_) : 0.0F;
    }
    ...
    public boolean shouldBlockExplode(Explosion p_38669_, BlockGetter p_38670_, BlockPos p_38671_, BlockState p_38672_, float p_38673_) {
        return (!this.isPrimed() || !p_38672_.is(CelestialTags.Blocks.MAG_RAIL) && !p_38670_.getBlockState(p_38671_.above()).is(CelestialTags.Blocks.MAG_RAIL)) && super.shouldBlockExplode(p_38669_, p_38670_, p_38671_, p_38672_, p_38673_);
    }
