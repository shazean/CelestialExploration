...
public class AstralHoglin extends Hoglin {
    ...
    public void finishConversion(ServerLevel serverLevel) {
        Zoglin zoglin = this.convertTo(CelestialEntities.VOIDED_ZOGLIN.get(), true);
        if (zoglin != null) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel p_149900_, AgeableMob p_149901_) {
        Hoglin hoglin = CelestialEntities.ASTRAL_HOGLIN.get().create(p_149900_);
        if (hoglin != null) {
    ...
    public boolean isFood(ItemStack p_34562_) {
        return p_34562_.is(CelestialItems.SULFUR_CRYSTAL.get());
    }
}
