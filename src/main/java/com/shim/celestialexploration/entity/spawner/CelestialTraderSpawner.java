...
public class CelestialTraderSpawner implements CustomSpawner {
    private static final float MIN_SPAWN_CHANCE = 0.25F;
    private static final float MAX_SPAWN_CHANCE = 0.75F;
    private static final float SPAWN_CHANCE_INCREASE = 0.10F;
    private final Random random = new Random();
    private static final int TICK_DELAY = 24000;
        ...
        if (!level.getGameRules().getBoolean(GameRules.RULE_DO_TRADER_SPAWNING)) {
            ...
            if (nextTick > 0) {
                ...
                if (!level.getGameRules().getBoolean(GameRules.RULE_DOMOBSPAWNING)) {
                    ...
                    float currentChance = this.spawnChance;
    ...
    private boolean spawn(ServerLevel level) {
        ...
        if (player == null) {
            ...
            if (blockpos2 != null && this.hasEnoughSpace(level, blockpos2)) {
                if (level.getBiome(blockpos2).is(Biomes.THE_VOID)) return false;
                if (!level.getBiome(blockpos2).is(CelestialTags.Biomes.CELESTIAL_BODIES)) return false;
